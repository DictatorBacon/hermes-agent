...
class SessionDB:
    ...
    def get_forward_compression_lineage(self, session_id: str) -> List[str]:
        """Return ``session_id`` plus its forward-only compression continuations.

        Branches, delegates, subagent/tool children, and ancestors are never
        included. The child-selection rules intentionally match resume routing.
        """
        if not session_id:
            return []
        current = session_id
        ...
        # pathological and should not happen in practice.
        for _ in range(100):
            with self._lock:
                cursor = self._conn.execute(
                    """
    ...
    def get_compression_tip(self, session_id: str) -> Optional[str]:
        ...
        continuations from branches/subagents by timestamp ordering, which is
        race-prone. Forward lineage uses structural provenance and the same
        child preference as resume routing instead.
        """
